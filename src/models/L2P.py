...
class Pool(nn.Module):
    ...
    def forward(self, x_embed, cls_features=None, used_prompts_frequency=None):
        ...
        # similarity = self.mech.add_noise(similarity)
        # print("adding noise eps 1")
        # drop_p = 0.2  # example
        # mask = (torch.rand_like(similarity) > drop_p).float()
        # similarity = similarity * mask
        _, idx = torch.topk(similarity, k=self.top_k, dim=1) # B, top_k
#         B = x_embed_mean.shape[0]
#         pool_size = self.prompt.shape[0]

#         # randomly sample top_k prompts for each batch item
#         idx = torch.randint(low=0, high=pool_size, size=(B, self.top_k), device=self.prompt.device)
